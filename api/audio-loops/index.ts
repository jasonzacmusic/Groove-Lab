...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  const conditions = [];
  if (artist) conditions.push(eq(audioLoops.artist, artist as string));
  if (genre) conditions.push(eq(audioLoops.genre, genre as string));
  if (feel) conditions.push(eq(audioLoops.feel, feel as string));
  if (instrument_category) conditions.push(eq(audioLoops.instrumentCategory, instrument_category as string));
  if (time_signature) conditions.push(eq(audioLoops.timeSignature, time_signature as string));
  if (section_type) conditions.push(eq(audioLoops.sectionType, section_type as string));
  if (key) conditions.push(eq(audioLoops.keySignature, key as string));
